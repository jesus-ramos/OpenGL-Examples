...
int init()
{
    ...
    if (!bind_uniform("transform_mat", &uniform_transform_mat))
        return 0;
...
void display()
{
    ...
    glUniformMatrix4fv(uniform_transform_mat, 1, GL_FALSE, mat);
...
void idle()
{
    move = sinf(glutGet(GLUT_ELAPSED_TIME) / 1000.0 * 2 * M_PI / 5);
